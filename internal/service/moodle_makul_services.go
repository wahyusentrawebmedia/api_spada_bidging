...
func (s *MoodleMakulService) SyncMakulAll(reqs []response.MoodleMakulRequest, parent string, db *gorm.DB) ([]model.Course, []model.MdlGroups, []error) {
	var courses []model.Course
	var groups []model.MdlGroups
	var errs []error
	for _, req := range reqs {
		if course, group, err := s.SyncMakul(req, parent, db); err != nil {
	...
	return courses, groups, errs
}

// SyncMakul syncs a course (makul) in Moodle based on the provided request data.
func (s *MoodleMakulService) SyncMakul(req response.MoodleMakulRequest, parent string, db *gorm.DB) (*model.Course, *model.MdlGroups, error) {
	...
	if err != nil {
		return nil, nil, errors.New("error mencari course dengan idnumber " + idnumber + ": " + err.Error())
	}

	if repoCourseData != nil && repoCourseData.ID > 0 {
		// Update existing course
		repoCourseData.FullName = req.NamaMK + " - " + req.Kelas + " - " + req.Tahun
		repoCourseData.ShortName = req.ShortNameMakul
		...
		if err := repoCourse.Update(nil, repoCourseData); err != nil {
			return nil, nil, errors.New("error mengupdate course dengan idnumber " + idnumber + ": " + err.Error())
		}
	} else {
		// Create new course
		newCourse := &model.Course{
			FullName:  req.NamaMK + " - " + req.Kelas + " - " + req.Tahun,
			ShortName: req.ShortNameMakul,
		...
		if err := repoCourse.Create(nil, newCourse); err != nil {
			return nil, nil, errors.New("error membuat course dengan idnumber " + idnumber + ": " + err.Error())
		}
	...
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil, errors.New("error mencari context course dengan instanceid " + fmt.Sprintf("%d", repoCourseData.ID) + ": " + err.Error())
	}
	if context == nil || context.ID == 0 {
		...
		if err := repoContext.Create(nil, newContext); err != nil {
			return nil, nil, errors.New("error membuat context course dengan instanceid " + fmt.Sprintf("%d", repoCourseData.ID) + ": " + err.Error())
		}
	}
	...
	idNumberGroup := req.KodeKelas
	group, err := repoGroups.GetByIDNumber(idNumberGroup)
	if err != nil {
		return nil, nil, errors.New("error mencari group dengan idnumber " + idNumberGroup + ": " + err.Error())
	}
	...
	if group != nil && group.ID > 0 {
		...
		if err := repoGroups.Update(nil, group); err != nil {
			return nil, nil, errors.New("error mengupdate group dengan idnumber " + idNumberGroup + ":, id : " + fmt.Sprintf("%d", group.ID) + " " + err.Error())
		}
	} else {
		// Create new group
		...
		if err := repoGroups.Create(nil, newGroup); err != nil {
			return nil, nil, errors.New("error membuat group dengan idnumber " + idNumberGroup + ": " + err.Error())
		}

		group = newGroup
	}

	return repoCourseData, group, nil
}
